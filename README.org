...
Cygwin の環境に gnupack-13 系を利用していた場合、提供される .bashrc には stty の
設定が含まれており、これが cygsh 実行時にエラーを出力します。
...
gnupack では startup_config.ini で環境変数の設定が可能となっています。しかし、cygsh
ではその設定が有効になりません。startup_config.ini で PATH などの環境設定を設定して
いる場合は、.bashrc に設定を移すなど、対応が必要となります。

**** ● Cygwin のコマンドは WSL の LANG 環境変数の値で実行します

cygsh では Cygwin のコマンドを WSL の LANG 環境変数の値で実行します。これは、コマンド
出力の文字化けを回避のために行っている対策です。もし、Cygwin を cp932 で使っていた場合
には、スクリプトの文字コードを変更するなど、調整が必要となる可能性があります。
