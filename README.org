...
DrvFs ファイルシステム上にシンボリックリンクを作成する場合は NTFS シンボリックリンク
...
Windows ショートカットファイルを作成するためのコマンドです。
...
フォルダのショートカットファイルを利用して cd をするには、次の設定が利用できます。
...
lcd でフォルダ移動後に元のフォルダに戻るには cd - が、lpd でフォルダ移動後に
元のフォルダに戻るには popd 利用できます。
...
ターミナルソフトのタイトルを起動するコマンド名で書き換えるコマンドです。

Fakeymacs にターミナルソフトで起動したコマンドを知らせ、キーバインドを切り替えれるように
するために作成しました。
...
Windows アプリである elecxzy を WSL2 から利用できるようにするコマンドです。
...
● elecxzy は、Microsoft Store 版のものではなく、次の GitHub からダウンロードしたものを
インストールしてください。Microsoft Store 版のアプリは、WSL2 から起動できないようです。

- https://github.com/kurouna/elecxzy

● elecxzy は、存在しないファイルをコマンドラインの引数として指定することはできないようです。
ファイルを作成してから elecxzy で指定するか、elecxzy を起動してからファイル（バッファ）を
作成するようにしてください。

** ■ zed

Windows アプリである Zed を --wsl オプションを利用し、WSL2 に接続して起動するためのコマンドです。
...
Windows アプリである Zed を --wsl オプションを利用せず、引数を wslpath で変換して起動するための
コマンドです。
