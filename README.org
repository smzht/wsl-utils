...
Cygwin の環境に gnupack-13 系を利用している場合、.bash_profile が存在しない
ために .bashrc が実行されていない可能性があります。この場合は、Cygwin の
ホームディレクトリに以下の内容の .bash_profile ファイルを作成してください。
...
また、gnupack-13 系で提供される .bashrc には stty の設定が含まれており、
これが cygsh 実行時にエラーを出力します。stty 文を以下のとおりに if 文で囲む
ことでエラーを回避できますので、必要であれば対策をしてください。
