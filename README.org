...
wslpath の代替コマンドです。WSL2 で UNC パスの変換も正常に行うコマンドとなっています。
...
パス名を Windows 形式で表示します。（デフォルトの動作）
...
（lnk ファイル）を作成できますが、このコマンドで作成された Windows ショートカットファイルは、
内部で利用している lnkinfo コマンドでは解析できないようです。
