...
上記の使用例の後者のコマンドをよく利用する場合は、以下の alias を .bashrc 等に
設定してご利用ください。
...
**** ● "tcgetpgrp failed: Not a tty" というメッセージが表示される場合があります

tty を必要とするコマンドの場合に出力されるようです。良い対策が見つからないため
未対策となっていますが、メッセージが表示されてもコマンドは正常に動作している
ようです。もし何か良い対策があれば、実施してみてください。
