...
Windows 10 1903 より前のバージョンの Windows でのショートカットファイルの作成ルールは、
以下のとおりとなります。
Windows 10 1903 以降では、VolFs にも \\\\wsl$\ で始まるパスで Windows からアクセス可能と
なりましたので、target ファイルが VolFs 上にあってもショートカットファイルを作成する
ことができます。

|---------------------------+-------------------------+------------------------------|
| target が示すのパスの位置 | shortcut を作成する場所 | 動作                         |
|---------------------------+-------------------------+------------------------------|
| DrvFs 配下                | DrvFs 配下              | ショートカットファイルを作成 |
| DrvFs 配下                | VolFs 配下              | ショートカットファイルを作成 |
| VolFs 配下                | DrvFs 配下              | 許可しない（エラーとする）   |
