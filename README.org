...
（Fall Creators Update までは、sudo mount -a
が必要となります。）
...
#+BEGIN_EXAMPLE
\\134172.16.0.1\<共有名>	/mnt/share1	drvfs	defaults,noatime,uid=1000,gid=1000	0 0
#+END_EXAMPLE
...
cygwin のコンソールを立ち上げるとエラーは出なくなるのですが、その他の解決
方法は分かっていません。
