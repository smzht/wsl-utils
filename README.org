...
wslcommon と wslinit は、コマンドと同じ場所（コマンドパスが通ったディレクトリ）に格納して
ご利用ください。
...
emacs が emacsclient と通信できるようになっており、helm と 必要であれば migemo の
設定を行っていれば、利用可能と思います。
...
cd "$(find * -type d -name '.*' -prune -o -type d -print | epeco)"
emacsclient -n "$(find * -type d -name '.*' -prune -o -type d -print | epeco)"
